import { useActiveIdContext, useJobItemDetails } from "../lib/hooks";
import BookmarkIcon from "./BookmarkIcon";
import Spinner from "./Spinner";

export default function JobItemContent() {
  const { activeId } = useActiveIdContext();
  const [isLoading, jobItemDetails] = useJobItemDetails(activeId);

  if (isLoading) {
    return <LoadingJobContent />;
  }

  // show empty job content when there are no job item details
  if (!jobItemDetails) {
